package com.example.josuecadillo.semana07sqltables.dao;
...
public class CompanyDAO {
    ...
    public Company updateCompany(Long companyId, String name, String address, String website,
                                 String phoneNumber) {
        ContentValues values = new ContentValues();
        values.put(DBHelper.COLUMN_COMPANY_NAME, name);
        values.put(DBHelper.COLUMN_COMPANY_ADDRESS, address);
        values.put(DBHelper.COLUMN_COMPANY_WEBSITE, website);
        values.put(DBHelper.COLUMN_COMPANY_PHONE_NUMBER, phoneNumber);
        long updateId = mDatabase
                .update(DBHelper.TABLE_COMPANIES, values,DBHelper.COLUMN_COMPANY_ID + " = " + companyId , null);
        Cursor cursor = mDatabase.query(DBHelper.TABLE_COMPANIES, mAllColumns,
                DBHelper.COLUMN_COMPANY_ID + " = " + companyId, null, null,
                null, null);
        cursor.moveToFirst();
        Company updateCompany = cursorToCompany(cursor);
        cursor.close();
